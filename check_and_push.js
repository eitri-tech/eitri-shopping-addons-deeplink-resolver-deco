...
const MANAGER_URL =
  "https://api.eitri.tech/eitri-manager-api/v2/revisions?eitriAppId=";
const BLIND_GUARDIAN_URL =
  "https://api.eitri.tech/blind-guardian-api/v2/o/auth";
const DEV_ENV_ID = "11c15532-150b-4c79-843b-17b8516bb94f"; // não pode ser o código do ambiente, deve ser o ID
...
function listProjects(directoryPath) {
  return fs
    ...
    .filter((folder) =>
      fs.existsSync(path.join(directoryPath, folder, "eitri-app.conf.js")),
    );
}
...
async function getPublishedVersions(eitriAppId, token) {
  try {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(
        error.message || `Erro ao buscar versões para o ID ${eitriAppId}`,
      );
    }
    ...
    console.error(
      `Erro ao buscar versões para o ID ${eitriAppId}:`,
      error.message,
    );
...
function getProjectConfig(directoryPath) {
  ...
  return {
    version,
    id,
    sharedVersion: isShared,
    message: messageVersion || versionMessage,
  };
}
...
async function checkAndPushInDirectory(directoryPath, token) {
  ...
  try {
    ...
    if (updatedProjects.length > 0) {
      const orderedList = updatedProjects.sort((a, b) =>
        a.sharedVersion ? -1 : 1,
      );
      for (const project of orderedList) {
        try {
          ...
          await publishProject(
            ...
            project.sharedVersion,
            project.message,
          );
          console.log(`${project.project} Publicado!`);
        } catch (error) {
          console.error(
            `${project.project} Erro ao atualizar o projeto:`,
            error.message,
          );
          hasError = true;
        }
