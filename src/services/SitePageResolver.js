...
const extractQueryFilters = deeplink => {
	...
	const facets = []
	let sort = ''
	for (const [key, value] of new URLSearchParams(query.split('#')[0]).entries()) {
...
const fetchPageEntry = async (template, key) => {
	try {
		const response = await Eitri.http.get(template.replace('{path}', key), { timeout: REQUEST_TIMEOUT_MS })
...
export const resolveDeeplinkFromSitePages = async deeplink => {
	...
	if (typeof template !== 'string' || !template.includes('{path}') || !deeplink) return false
	...
	for (let hop = 0; hop <= MAX_REDIRECTS; hop++) {
		...
		const pageFacets = Array.isArray(entry.facets) ? entry.facets.filter(f => f?.key && f?.value) : []
		...
		openEitriApp('home', {
			...
			params: { facets: [...pageFacets, ...query.facets], ...(sort ? { sort } : {}) }
		})
		return true
	}
